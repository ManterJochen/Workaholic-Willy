...
| `python -m src.robot.execution.real_cell`, the operator console, and [`examples/simulation/01_rehearse_a_pick.py`](../../examples/simulation/01_rehearse_a_pick.py) | `from_robot_config`, through `Cell` and those two builders | the terminal, the browser and a Python script drive one construction |
...
rehearsal from Python in
[`examples/simulation/01_rehearse_a_pick.py`](../../examples/simulation/01_rehearse_a_pick.py).
...
[`examples/simulation/03_isaac_pick_rate.py`](../../examples/simulation/03_isaac_pick_rate.py) runs the known-pose runner from Python and checks
