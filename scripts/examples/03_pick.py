"""One grasp, end to end, and a measured account of which layers ran.

    python scripts/examples/03_pick.py                       # rehearse on a dummy arm
    python scripts/examples/03_pick.py --live                # drive the real cell
    python scripts/examples/03_pick.py --live --prompt "a red cube"
...
claim and the check are one thing. The layers line under the pick works the same way: it is read
off report fields that stay `None` when a layer produced nothing, so a capability that was not
exercised cannot be reported as demonstrated.
...
`MotionStatus.INVALID_TARGET`. The resolver is built from the calibration artifact
`02_calibration.py` writes, which is why an uncalibrated cell fails here rather than in the
...
def main(argv: list[str] | None = None) -> int:
    ...
    with Example("03 pick", what, live=args.live, profile=args.profile) as run:
        ...
        if pick.passed:
            run.note("")
            run.note("Next: 06_full_pipeline.py, the same path with calibration and a corpus "
                     "in front of it.")
        else:
