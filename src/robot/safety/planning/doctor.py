...
def _probe_curobo(blocks: tuple[str, ...], robot_config: str) -> tuple[Probe, ...]:
    ...
    if robot_config.startswith("<none"):
        probes.append(Probe(
...
def _probe_gripper(model: str, gripper: str | None) -> tuple[Probe, ...]:
    ...
    geometry refuses to build without it. The bundle and the map follow from the name exactly
    as the guard derives them, through ``hand.py``.
    """
    ...
    if not gripper:
        return (Probe(
            "gripper geometry",
            ProbeStatus.MISSING,
            "robot.gripper.model is unset, so this cell names no hand: a cell whose guard reads hand "
            "geometry refuses to build, and no bundle or sphere map can be probed for a hand nobody named",
            "name the hand in the cell profile: robot.gripper.model: <registry name>, for example "
            "robotiq_2f85",
        ),)
    try:
        load_gripper(gripper, aliases=False)
...
def run_doctor(
    *, model: str = "ur5e", robot_config: str | None = None, gripper: str | None = None
    ...
    logger.info("motion-stack doctor starting for model %r, gripper %r (robot config %s)",
                model, gripper or "<unset>", robot_config or "<default>")
    probes = (
        _probe_coal(blocks),
