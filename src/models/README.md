...
| [`speech/`](speech/README.md) | `SpeechEngine`, `Transcript`, `WhisperTransformersEngine`, `SpeechGate`, `Listener`: speech to text behind one protocol, answering with a frozen report (text, detected language, duration, engine, weights, device, latency). Whisper large-v3-turbo on transformers is the engine, and Silero VAD scores a recording before Whisper is asked, because Whisper answers silence with a word. The operator console reaches it over `POST /v1/voice/transcribe` and its push to talk `POST /v1/voice/listen`; `Listener.listen()` cuts utterances from the cell PC's microphone and has no caller yet. `Confirmation` is a person's answer before any words become a prompt. |
