...
| [`07_calibrate_a_fixed_camera.py`](real_robot/07_calibrate_a_fixed_camera.py) | where a fixed camera sits in the robot's frame, from an automatic sweep: check, dry run, sweep, with a line per pose saying whether it counted and why not | the arm, in the sweep |
...
| [`09_calibrate_a_wrist_camera.py`](real_robot/09_calibrate_a_wrist_camera.py) | the same automatic sweep for a camera on the flange that looks where the tool points | the arm, in the sweep |
| [`10_calibrate_a_wrist_camera_with_fixed_poses.py`](real_robot/10_calibrate_a_wrist_camera_with_fixed_poses.py) | the wrist-camera solve with the CAMERA aimed at one marker lying flat at a known place (`MarkerAim`): one look from where the arm stands estimates where a tilted camera sits on the tool, then seventeen views round the marker, each tilting the tool as little as it can from its heading and rolling the camera about its line of sight, screened against the box, the arm's reach and its joint window, ordered by joint travel and re-aimed as the marker is seen; a heading that admits fewer views than the solve needs gives way to the one of `-y`, `y`, `x`, `-x` that admits the most, and the report says which; or a JSON file of your own stations, where a station may be the joint angles you taught on the pendant (`{"joints_deg": [...]}`, as in [`eih_fixed_stations.json`](real_robot/eih_fixed_stations.json)), sent as one judged joint move | the arm, in the sweep |
