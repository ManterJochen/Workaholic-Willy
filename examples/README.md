...
| [`11_teach_poses_by_hand.py`](real_robot/11_teach_poses_by_hand.py) | joint poses taught by guiding the arm by hand for one camera, `CAMERA` (the primary when `None`), the arm built alone: the payload confirmed first, a name per pose (Enter takes `LOOK_1`, `LOOK_2`, ...), Enter captures once the arm stands still and prints one line to paste that names the camera it belongs to (`LOOK_1 = JointPositions.deg(...)  # rig 'wrist', eye in hand`, the look poses of 12), each pose also added to `logs/taught_poses.json` with its rig and mounting; with `SHOW_CAMERA` a window of that camera shows the pose and the count, red outside a boundary; outside the cable window or the box it says so and does not capture, and never holds the arm for it | nothing by itself: you move the arm |
| [`12_pick_with_the_camera.py`](real_robot/12_pick_with_the_camera.py) | a campaign of camera picks on one part: your own pick motion, look poses declared as joints in degrees and tried in order, the part put back where it was grasped after every lift, where the object and the grasp were printed per attempt, a verdict, a JSONL record and a saved overlay image per attempt (weights: `dino-tiny sam2`); with `SHOW_CAMERAS` every camera of the cell live in a window of its own, each attempt's grasp overlay pinned on the primary's (`PickRun.from_cell(..., view=LiveView(...))`) | the arm and the hand |
| [`13_pick_and_place_with_the_camera.py`](real_robot/13_pick_and_place_with_the_camera.py) | pick what one prompt names and set it down on what a second one names, both found by camera, with no coordinate in the program: every camera the world is built from opened (16) with a `Locator` each over one set of models (`Locator.for_cameras`), the first camera that locates a prompt answers it, a wrist camera from each look pose in turn; the set-down measured from the target's top, the part's hang below the grasp from the declared table (never the part's lowest seen point, so it is never pressed in; a part off a raised block drops further) and at least 5 mm of air (`located.set_down`), the target held out of the camera world for the place (`robot.place(..., keep_out=)`); a target nobody finds leaves the part held (weights: those of 12); with `SHOW_CAMERAS` every camera live in a window of its own, each locate's masks, labels and centres pinned on its camera's window (`Locator.for_cameras(..., view=)`) | the arm and the hand |
| [`14_speak_a_command.py`](real_robot/14_speak_a_command.py) | a spoken command, confirmed by a person, becomes the pick prompt (weights: `whisper-turbo silero-vad`, and those of 12) | the arm and the hand |
| [`15_speak_pick_and_hand_handover.py`](real_robot/15_speak_pick_and_hand_handover.py) | speech (14), a camera pick step by step (`Locator`, the part's grasps, `robot.pick`), then the arm brings the part to where the camera sees the person's hand | the arm and the hand |
| [`16_a_cell_with_several_cameras.py`](real_robot/16_a_cell_with_several_cameras.py) | which rigs feed the live planner world and why the others do not, then one owner per device handed to the arm | the arm, and two or more cameras |
| [`17_pick_with_fused_cameras.py`](real_robot/17_pick_with_fused_cameras.py) | a campaign of camera picks on a cell whose cameras are fused into one cloud per object before each grasp: the tree asked first (`CameraFusionPlan`) and refused in one sentence naming what is missing when it cannot fuse (a one-camera cell is), a look that keeps the arm out of the cameras' view, then per attempt which cameras the grasp was planned from (`fused_views`) and how many objects of that frame were fused (`fused_objects`), a verdict and a JSONL record (weights: `dino-tiny sam2`); with `SHOW_CAMERAS` every camera live in a window of its own, each attempt's grasp overlay pinned on the primary's | the arm and the hand, with two or more cameras |
